...
export default function EquipmentPage({ page, products = [], projects = [], loading = false, error = false }) {
  ...
  return <main className={styles.page}>
    ...
    {!!projects.length && <section className={styles.section} id="projects"><div className={styles.heading}><p className={styles.eyebrow}>В реализованной среде</p><div><h2>Примеры пространств</h2><p>Посмотрите, как похожие игровые сценарии и задачи благоустройства воплощены в проектах Людно.</p></div></div><div className={styles.projectGrid}>{projects.map(project => <a href={project.href} key={project.id}>{project.image && <img src={imageUrl({ image: project.image })} alt={project.title} loading="lazy" width="720" height="480" />}<h3>{project.title} ↗</h3></a>)}</div></section>}
